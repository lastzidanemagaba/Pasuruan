...
<body>
    ...
    <table class="table table-bordered">
		...
		<?php
            ...
            $query = "SELECT tindak_pidana.nama AS nama_tindak_pidana,usia_pelaku.nama as nama_group , 
			COUNT(*) AS total_count 
			FROM `tb_tersangka` pelaku 
			JOIN tb_lp lp ON pelaku.id = lp.id 
			JOIN tb_usia_pelaku usia_pelaku ON pelaku.id_usia_pelaku = usia_pelaku.id 
			JOIN tb_tindak_pidana tindak_pidana ON lp.id_tindak_pidana = tindak_pidana.id 
			group by usia_pelaku.nama, lp.id_tindak_pidana;";
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                for ($i = 0; $i < count($tempat); $i++) {
					...
					JOIN tb_usia_pelaku usia_pelaku ON pelaku.id_usia_pelaku = usia_pelaku.id
					JOIN tb_tindak_pidana tindak_pidana ON lp.id_tindak_pidana = tindak_pidana.id
					WHERE tindak_pidana.nama = '" . $row['nama_tindak_pidana'] . "' AND usia_pelaku.nama = '" . $tempat[$i] . "';";
			...
			for ($i = 0; $i < count($tempat); $i++) {
				// Query SQL untuk mengambil data dari hasil query yang telah Anda berikan
				$query = "SELECT tindak_pidana.nama AS nama_tindak_pidana,usia_pelaku.nama as nama_group , COUNT(*) AS total_count FROM `tb_tersangka` pelaku JOIN tb_lp lp ON pelaku.id = lp.id JOIN tb_usia_pelaku usia_pelaku ON pelaku.id_usia_pelaku = usia_pelaku.id JOIN tb_tindak_pidana tindak_pidana ON lp.id_tindak_pidana = tindak_pidana.id group by usia_pelaku.nama, lp.id_tindak_pidana;";
...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js"></script>
    <script>
        function exportToExcel() {
            ...
            XLSX.writeFile(wb, 'LP Usia Tersangka ' + namefiletime + '.xlsx');
        }
