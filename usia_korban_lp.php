...
<body>
    ...
    <table class="table table-bordered">
		...
		<?php
            ...
            $query = "SELECT tindak_pidana.nama AS nama_tindak_pidana,usia_korban.nama as nama_group , 
			...
			group by usia_korban.nama, lp.id_tindak_pidana;";
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                for ($i = 0; $i < count($tempat); $i++) {
					...
					$query = "SELECT COUNT(*) AS total_count 
							  ...
							  JOIN tb_usia_korban usia_korban ON korban.id_usia_korban = usia_korban.id 
							  JOIN tb_tindak_pidana tindak_pidana ON lp.id_tindak_pidana = tindak_pidana.id 
							  WHERE usia_korban.nama = '" . $tempat[$i] . "' AND tindak_pidana.nama = '" . $row['nama_tindak_pidana'] . "';";
			...
			for ($i = 0; $i < count($tempat); $i++) {
				...
				$query = "SELECT COUNT(*) AS total_count 
						  ...
						  JOIN tb_usia_korban usia_korban ON korban.id_usia_korban = usia_korban.id 
						  JOIN tb_tindak_pidana tindak_pidana ON lp.id_tindak_pidana = tindak_pidana.id 
						  WHERE usia_korban.nama = '" . $tempat[$i] . "';";
